...
;(function(definition) {
  ...
  CIQ.MenuManager.menuOff = function(closeAll, dontBlur) {
    ...
    if (!CIQ.MenuManager.registeredCharts.length) return
    if (!dontBlur && document.activeElement && document.activeElement.tagName != 'BODY' && document.activeElement.blur)
      document.activeElement.blur() // Hide keyboard on touch devices
  ...
  CIQ.DrawingToolbar.prototype.initialize = function(htmlElement) {
    ...
    if (this.stx.currentVectorParameters.annotation.font.family)
      CIQ.DrawingToolbar.setFont(htmlElement, this.stx.currentVectorParameters.annotation.font.family)
    ...
    for (var i = 0; i < results.length; i++) {
      ...
      if (result.exchange)
        exchangeSpan.appendChild(
          this.config.stx ? CIQ.translatableTextNode(this.config.stx, result.exchange) : result.exchange,
        )
  ...
  CIQ.Studies.studyDialog = function(stx, study, div, override) {
    ...
    for (i in sd.inputs) {
      ...
      var defaultValue =
        override && override.inputs && override.inputs[i] !== null && typeof override.inputs[i] != 'undefined'
          ? override.inputs[i]
          : acceptedData
      if (acceptedData.constructor == Number) {
        ...
        if (
          ...
          for (var field in chart.dataSet[chart.dataSet.length - 1]) {
            if (
              ['Open', 'High', 'Low', 'Close', 'Adj_Close', 'hl/2', 'hlc/3', 'hlcc/4', 'ohlc/4'].indexOf(field) == -1
            ) {
